...
/* Copy constructor */
HttpMessage::HttpMessage(const HttpMessage &original) { *this = original; }
...
const std::string &HttpMessage::getBody(void) const { return (this->_body); }
...
HttpMessage::e_mimeType HttpMessage::getMimeType(const std::string &filePath)
{
    ...
    if (pos == std::string::npos) return APPLICATION_OCTET_STREAM;

    it = HttpMessage::_extToMimeMap.find(filePath.substr(pos));
    if (it != HttpMessage::_extToMimeMap.end()) return it->second;
