...
/*   Updated: 2024/05/19 22:32:54 by hnogared         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string readFile(const std::string &filePath)
{
    try
    {
        ...
        if (!fileStream.is_open()) throw std::exception();
    ...
    catch (const std::exception &e)
    {
        throw webserv::FileException(
            "readFile(): '" + filePath + "': " + std::strerror(errno), errno);
    }
}
...
std::string joinPaths(const std::string &path1, const std::string &path2)
{
    ...
    joinedPath = path1;
    if (path1[path1.size() - 1] != '/') joinedPath += '/';
...
{
    if (entry1.d_type != entry2.d_type) return (entry1.d_type < entry2.d_type);
...
std::vector<dirent> readDirectory(const std::string &path)
{
    ...
    if (!dir)
    {
        throw webserv::RuntimeError(
            "opendir(): '" + path + "': " + std::string(strerror(errno)),
            errno);
    }

    while ((entry = readdir(dir))) entries.push_back(*entry);
