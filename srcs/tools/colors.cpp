...
/*   Updated: 2024/05/19 22:32:40 by hnogared         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (!os.good()) throw std::runtime_error("Stream is not in a good state");

    os << ESQ_SEQ << static_cast<int>(format) << "m";

    if (!os.good()) throw std::runtime_error("Failed to write to stream");
    return (os);
}
