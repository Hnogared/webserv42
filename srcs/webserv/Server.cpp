...
/*   Updated: 2024/05/19 22:34:20 by hnogared         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Destructor */
Server::~Server(void) { this->_cleanup(); }

/* ************************************************************************** */
/* Getters */

Harl &Server::getLogger(void) { return (this->_logger); }
...
void Server::start(void)
{
    ...
    std::map<std::pair<std::string, int>,
             VirtualServerManager *>::const_iterator it;
    ...
    while (this->_running)
    {
        ...
        {
            if (!(pollfdIt->revents & (POLLIN | POLLHUP | POLLERR))) continue;
...
{
    std::map<std::pair<std::string, int>,
             webserv::VirtualServerManager *>::iterator it;
...
void Server::_init(const std::string &configPath)
{
    ...
    this->_logger.complain(Harl::INFO,
                           "Logging at " + this->_logger.getLogFilePath());
    this->_logger.complain(Harl::INFO,
    ...
    catch (const ConfigurationParser::ConfigException &e)
    {
        ...
        if (configPath == WS_DFL_CONFIG_PATH)
        {
            this->_logger.log(Harl::ERROR,
                              "Invalid default configuration file: " +
                                  configPath + ": " + e.what());
...
void Server::_initVirtualServerManagers(
    const std::vector<Configuration> *configs)
{
    ...
    try
    {
        ...
        for (specificIt = specificConfigs.begin();
             specificIt != specificConfigs.end(); specificIt++)
...
void Server::_initVirtualServer(const Configuration &config)
{
    ...
    {
        this->_managers[key] =
            new VirtualServerManager(this->_protocol, &this->_logger);
    }
...
void Server::_updateFds(std::vector<pollfd> &fds)
{
    ...
    if (fds.capacity() != size) fds.resize(size);
...
void Server::_cleanup(bool removeLockFile)
{
    ...
    if (removeLockFile) std::remove(WS_LOCK_FILE);
}
