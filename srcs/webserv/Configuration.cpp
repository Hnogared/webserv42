...
Configuration::Configuration(void)
    : _protocol(WS_HTTP_VERSION),
      _backlog(WS_DFL_BACKLOG),
      _root(""),
      _index(WS_DFL_INDEX),
...
sockaddr_in &Configuration::getAddress(void) { return (this->_address); }
...
unsigned int Configuration::getBacklog(void) const { return (this->_backlog); }
...
std::string Configuration::getRoot(void) const { return (this->_root); }

std::string Configuration::getIndex(void) const { return (this->_index); }
...
void Configuration::setPort(int port) { this->_address.sin_port = htons(port); }
...
void Configuration::setIndex(const std::string &index) { this->_index = index; }
...
std::ostream &Configuration::print(std::ostream &os) const
{
    ...
    if (!this->_errorPages.empty())
    {
        ...
        os << "\n\nError redirects";
        for (it = this->_errorPages.begin(); it != this->_errorPages.end();
             it++)
            os << "\n └ " << it->first << " -> " << it->second;
    }
