...
Socket VirtualServerManager::getSocket(void) const { return (this->_socket); }
...
const std::vector<VirtualServer *> &VirtualServerManager::getServers(void) const
{
...
{
    if (client) this->_clients.push_back(client);
}
...
bool VirtualServerManager::tryServeFd(int fd, short revents)
{
    ...
    for (clientIt = this->_clients.begin(); clientIt != this->_clients.end();
         clientIt++)
    {
        if (fd != (*clientIt)->getSocket().getFd()) continue;
...
void VirtualServerManager::_initSocket(const Configuration &config)
{
    ...
    if (this->_socket.getFd() < 0 ||
        setsockopt(this->_socket.getFd(), SOL_SOCKET, SO_REUSEPORT, &optval,
                   sizeof(optval)) < 0)
    {
...
void VirtualServerManager::_acceptConnection(void)
{
    ...
    if (client_fd == -1)
    {
        this->_log(Harl::ERROR, NULL,
                   "Failed client connection: " + std::string(strerror(errno)));
        return;
    }
    ...
    catch (const std::exception &e)
    {
        close(client_fd);
        this->_log(Harl::ERROR, NULL,
                   "Failed client connection: " + std::string(e.what()));
        return;
    }
    ...
    std::vector<webserv::Client *>::iterator clientIt)
{
    ...
    try
    {
        ...
        if (this->_defaultServer->tryHandleClientRequest(
                *client, this->_catchAllServer == this->_defaultServer))
            return;
