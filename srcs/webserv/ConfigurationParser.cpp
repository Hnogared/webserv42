    ...
    while (pos < line.size())
    {
        switch (line[pos])
        {
            ...
            default:
                ...
                tokens.push(
                    (t_token){lineNbr, STRING, line.substr(pos, end - pos)});
                pos = end;
        }
    }
}

void ConfigurationParser::_parseTokens(
    std::queue<t_token> &tokens, std::vector<Configuration> &configurations)
{
    t_token token;
...
void ConfigurationParser::_parseHttpConfig(
    std::queue<t_token> &tokens, std::vector<Configuration> &configurations)
{
    t_token token;
    ...
    while (!tokens.empty())
    {
        ...
        if (token.type == CLOSE_BRACE) break;
    ...
    if (token.type != CLOSE_BRACE) throw MissingToken("http", "}");
}

void ConfigurationParser::_parseServerConfig(
    std::queue<t_token> &tokens, std::vector<Configuration> &configurations)
{
    t_token token;
                                             ...
                                             Configuration &config)
{
    ...
    if (token.type != STRING) throw UnexpectedToken(token, "listen", "string");
    ...
    if (token.type != SEMICOLON) throw UnexpectedToken(token, "listen", ";");
}
                                           ...
                                           Configuration &config)
{
    ...
    if (token.type != STRING) throw UnexpectedToken(token, "root", "string");
    if (tokens.front().type != SEMICOLON)
                                            ...
                                            Configuration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("index", "string");

    token = tokens.front();
    tokens.pop();

    if (token.type != STRING) throw UnexpectedToken(token, "index", "string");
    if (tokens.front().type != SEMICOLON)
                                              ...
                                              Configuration &config)
{
    ...
    if (token.type != STRING) throw UnexpectedToken(token, "backlog", "string");
    if (tokens.front().type != SEMICOLON)
                                          ...
                                          LocationConfiguration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("return", "string");
    ...
    if (token.type != STRING) throw UnexpectedToken(token, "return", "string");
    ...
    if (token.type != SEMICOLON) throw UnexpectedToken(token, "return", ";");
}
                                        ...
                                        LocationConfiguration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("root", "string");

    token = tokens.front();
    tokens.pop();

    if (tokens.empty()) throw MissingToken("root", ";");
    if (token.type != STRING) throw UnexpectedToken(token, "root", "string");
    if (tokens.front().type != SEMICOLON)
        throw UnexpectedToken(tokens.front(), "root", ";");
                                         ...
                                         LocationConfiguration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("index", ";");
    if (token.type != STRING) throw UnexpectedToken(token, "index", "string");
    if (tokens.front().type != SEMICOLON)
        throw UnexpectedToken(tokens.front(), "index", ";");
                                            ...
                                            LocationConfiguration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("fastcgi_pass", "string");

    token = tokens.front();
    tokens.pop();

    if (tokens.empty()) throw MissingToken("fastcgi_pass", ";");
    if (token.type != STRING)
                                             ...
                                             LocationConfiguration &config)
{
    ...
    if (tokens.empty()) throw MissingToken("fastcgi_param", "string");
...
ConfigurationParser::ConfigException::operator=(const ConfigException &original)
{
...
ConfigurationParser::InvalidConfigFile::InvalidConfigFile(
    int lineNbr, const std::string &context, const std::string &message)
...
/* Token constructor */
ConfigurationParser::UnexpectedToken::UnexpectedToken(
    const t_token &token, const std::string &context)
    : ConfigException(tool::strings::toStr(token.lineNbr) + ": " + context +
...
ConfigurationParser::UnexpectedToken::UnexpectedToken(
    const t_token &token, const std::string &context, const std::string &exp)
    : ConfigException(tool::strings::toStr(token.lineNbr) + ": " + context +
                          ": Unexpected token `" + token.content +
                          "`. Expected `" + exp + "`",
                      11)
...
ConfigurationParser::UnexpectedToken::operator=(const UnexpectedToken &original)
{
    if (this == &original) return (*this);
    ConfigException::operator=(original);
    return (*this);
...
ConfigurationParser::MissingToken &ConfigurationParser::MissingToken::operator=(
